  ...
  if(mod == 3) {
    ...
    if(MODE_DATA32) {
      ...
      if(type != 7) {
        ...
        printf("x86emuOp_op_A_word_RM_R: src32=%lx addr=%lx mark=%d\n", src32, addr, REG_MARK(src32));
        MEM_TYPE_SET(addr, MEM_TYPE_GET(addr) || REG_MARK(src32));
      }      
    }
    else {
...
Handles opcode 0x40
****************************************************************************/
void x86emuOp_inc_AX(x86emu_t *emu, u8 op1)
{
  if(MODE_DATA32) {
    OP_DECODE("inc eax");
  ...
  if(mod == 3) {
    ...
    if(MODE_DATA32) {
      ...
      *dst32 = fetch_data_long(emu, addr);
      printf("x86emuOp_mov_word_R_RM dst32=%lx addr=%lx type=%d\n", dst32, addr, MEM_TYPE_GET(addr));
      REG_MARK(dst32) = MEM_TYPE_GET(addr);
    }
    else {
      dst16 = decode_rm_word_register(emu, rh);
      OP_DECODE(",");
...
#define FIX_FUNC_ARR(X) do { for (int i = 0; i < sizeof(X) / sizeof((X)[0]); ++i) (X)[i] = (u64)(X)[i] + 0xFFFFFF8009800000ull; } while (0)

void ops_fix_all(void)
{
  FIX_FUNC_ARR(x86emu_optab);
  FIX_FUNC_ARR(x86emu_optab2);

  FIX_FUNC_ARR(op_A_byte);
  FIX_FUNC_ARR(op_A_word);
  FIX_FUNC_ARR(op_A_long);

  FIX_FUNC_ARR(op_B_byte);
  FIX_FUNC_ARR(op_B_word);
  FIX_FUNC_ARR(op_B_long);
}